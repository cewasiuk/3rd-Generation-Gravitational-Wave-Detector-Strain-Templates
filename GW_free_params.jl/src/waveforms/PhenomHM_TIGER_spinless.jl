...
"""
function PolAbs(model::PhenomHM_TIGER_spinless,
    f::AbstractVector,
    mc,
    eta,
    chi1,
    ...
    hp, hc = waveform.hphc(model, f, mc, eta, chi1, chi2, dL, iota, o1)
...
ToDo: Need documentation
"""
function Pol(model::PhenomHM_TIGER_spinless,
    f::AbstractVector,
    mc,
    eta,
    chi1,
...
```
"""
function Phi(model::PhenomHM_TIGER_spinless,
    f,
    mc,
    eta,
    chi1,
...
```
"""
function Ampl(model::PhenomHM_TIGER_spinless,
    f,
    mc,
    eta,
    chi1,
...
```
"""
function hphc(model::PhenomHM_TIGER_spinless,
    f,
    mc,
    eta,
    chi1,
    ...
    # This function retuns directly the full plus and cross polarisations, avoiding for loops over the modes
        ...
        if !(model.PNorder in PNorder_available)
            error("The PN order modification is not available for this model.")
        end
    
        # Turn on the PN order term
        ...
        if model.PNorder == -1
            deltaVminus2 = o1
        elseif model.PNorder == 0
            deltaV0 = o1
        elseif model.PNorder == 0.5
            deltaV1 = o1
        elseif model.PNorder == 1
            deltaV2 = o1
        elseif model.PNorder == 1.5
            deltaV3 = o1
        elseif model.PNorder == 2
            deltaV4 = o1
        elseif model.PNorder == 2.5
            deltaV5 = o1
        elseif model.PNorder == log(2.5)
            deltaV5log = o1
        elseif model.PNorder == 3
            deltaV6 = o1
        elseif model.PNorder == log(3.)
            deltaV6log = o1
        elseif model.PNorder == 3.5
            deltaV7 = o1
        end
    ...
    TF2coeffs = TF2coeffsStructure_BGR(
        deltaVminus2,
        ...
        (3715.0 / 756.0 + (55.0 * eta) / 9.0)*(1+deltaV2),

        ((-16.0 * pi) *(1+deltaV3) +
        (113.0 * Seta * chi_a) / 3.0 +
        (113.0 / 3.0 - (76.0 * eta) / 3.0) * chi_s),

        (5.0 * (3058.673 / 7.056 + 5429.0 / 7.0 * eta + 617.0 * eta2) *(1+deltaV4) / 72.0 +
        ...
        (38645.0 * pi / 756.0 - 65.0 * pi * eta / 9.0) * (1+deltaV5) - (
            (732985.0 / 2268.0 - 24260.0 * eta / 81.0 - 340.0 * eta2 / 9.0) * chi_s +
            (732985.0 / 2268.0 + 140.0 * eta / 9.0) * Seta * chi_a
        ) ,

       ((38645.0 * pi / 756.0 - 65.0 * pi * eta / 9.0) *(1+deltaV5log) - (
            (732985.0 / 2268.0 - 24260.0 * eta / 81.0 - 340.0 * eta2 / 9.0) * chi_s +
            (732985.0 / 2268.0 + 140.0 * eta / 9.0) * Seta * chi_a
        )) * 3.0,
        ...
        -6848.0 / 21.0 *(1+deltaV6log) ,
        ((77096675.0 * pi / 254016.0 + 378515.0 * pi * eta / 1512.0 -
        74045.0 * pi * eta2 / 756.0) *(1+deltaV7) +
        (
    ...
    PhiInspcoeffs = PhiInspcoeffsStructure_BGR(
        ...
        TF2coeffs.minus_two * TF2OverallAmpl * (pi^(-7.0 / 3.0)),
        sigma1,
        sigma2 * 0.75,
        sigma3 * 0.6,
        sigma4 * 0.5,
    ...
    # Joining at fInsJoinPh
    # PhiIns (fInsJoinPh)  =   PhiInt (fInsJoinPh) + C1Int + C2Int fInsJoinPh
    # PhiIns'(fInsJoinPh)  =   PhiInt'(fInsJoinPh) + C2Int
    # This is the first derivative wrt f of the inspiral phase computed at fInsJoinPh, first add the PN contribution and then the higher order calibrated terms
    DPhiIns =
        (
            ...
            7.0 * TF2coeffs.minus_two * TF2OverallAmpl * ((pi * fInsJoinPh)^(-2.0 / 3.0))
    ...
    # This is the inspiral phase computed at fInsJoinPh
    PhiInsJoin =
    ...
    PhiInspcoeffs.min_seven_thirds * (fInsJoinPh^(-7. /3.)) +
    (
    ...
    PhDBconst =
        1.0 ./ Map_amPhi .* _completePhase_BGR(
            tmpMf,
            C1MRDHM,
            C2MRDHM,
            Rholm,
    ...
    tmpMf = reshape(tmpMf, 1, :)
    PhDCconst =
        1.0 ./ Map_arPhi .* _completePhase_BGR(
            tmpMf,
            C1MRDHM,
            C2MRDHM,
            Rholm,
...
useful functions for hphc()
"""
function _RDfreqCalc(model::PhenomHM_TIGER_spinless, finalmass, finalspin, l, m)
