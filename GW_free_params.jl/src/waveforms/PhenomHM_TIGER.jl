...
"""
function PolAbs(model::PhenomHM_TIGER,
    f::AbstractVector,
    mc,
    eta,
    chi1,
...
ToDo: Need documentation
"""
function Pol(model::PhenomHM_TIGER,
    f::AbstractVector,
    mc,
    eta,
    chi1,
...
```
"""
function Phi(model::PhenomHM_TIGER,
    f,
    mc,
    eta,
    chi1,
...
```
"""
function Ampl(model::PhenomHM_TIGER,
    f,
    mc,
    eta,
    chi1,
...
```
"""
function hphc(model::PhenomHM_TIGER,
    f,
    mc,
    eta,
    chi1,
    ...
    TF2coeffs = TF2coeffsStructure_BGR(
        ...
        (113.0 / 3.0 - (76.0 * eta) / 3.0) * chi_s)*(1+deltaV3),
        ...
        (240.0 / 9.6 * QuadMon2 - 7.0 / 9.6) * m2ByM^2  * chi22)*(1+deltaV4),
        TF2_5coeff_tmp*(1+deltaV5),
        TF2_5coeff_tmp * 3.0*(1+deltaV5log),
        (TF2_6coeff_tmp - (
            (326.75 / 1.12 + 557.5 / 1.8 * eta) * eta * chi1dotchi2 +
            (
                (4703.5 / 8.4 + 2935.0 / 6.0 * m1ByM - 120.0 * m1ByM^2 ) +
...
useful functions for hphc()
"""
function _RDfreqCalc(model::PhenomHM_TIGER, finalmass, finalspin, l, m)
